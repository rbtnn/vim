...
pum_redraw(void)
{
	...
	for (round = 0; round < 3; ++round)
	{
		...
		for ( ; ; MB_PTR_ADV(p))
		{
			...
			if (pum_rl)
			{
				...
				if (rt != NULL)
				{
				    ...
				    pum_screen_put_with_attr(row, col - size + 1 + TSB_LCOL(NULL), rt, (int)STRLEN(rt), hlf);
				    vim_free(rt_start);
				}
				vim_free(st);
			    }
...
#endif
			{
				...
				pum_screen_put_with_attr(row, col + TSB_LCOL(NULL), st, size, hlf);
				vim_free(st);
			    }
			    col += width;
			}
...
#endif
			{
			    screen_puts_len((char_u *)"  ", 2, row, col + TSB_LCOL(NULL), attr);
			    col += 2;
			}
			totwidth += 2;
	    ...
	    if (pum_rl)
	    {
		screen_fill(row, row + 1, pum_col - pum_base_width - n + 1 + TSB_LCOL(NULL),
						    col + 1 + TSB_LCOL(NULL), ' ', ' ', attr);
		col = pum_col - pum_base_width - n;
	    }
	    else
#endif
	    {
		screen_fill(row, row + 1, col + TSB_LCOL(NULL), pum_col + pum_base_width + n + TSB_LCOL(NULL),
							      ' ', ' ', attr);
		col = pum_col + pum_base_width + n;
	    }
...
#ifdef FEAT_RIGHTLEFT
	    if (pum_rl)
		screen_putchar(' ', row, pum_col - pum_width + TSB_LCOL(NULL),
