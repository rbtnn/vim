...
draw_tabsidebar()
{
	...
	for (vsrow = 1; vsrow < cmdline_row + 1; vsrow++)
	    screen_fill(vsrow - 1, vsrow,
		    (p_tsba ? TSB_COLUMNS() + 0 : maxwidth - 1),
		    (p_tsba ? TSB_COLUMNS() + 1 : maxwidth),
	...
	int	attr)
{
	...
	screen_fill(start_row, end_row,
		(p_tsba ? TSB_COLUMNS() : 0) + col,
		(p_tsba ? TSB_COLUMNS() : 0) + maxwidth,
	...
	else
	{
		...
		*pcol = col_end;
		...
		if ((TSBMODE_REDRAW == tsbmode) && (0 <= (*prow - offsetrow) && (*prow - offsetrow) < maxrow))
		    screen_puts(buf, *prow - offsetrow, *pcol + (p_tsba ? TSB_COLUMNS() : 0), attr);
		(*pcol) += chcells;
	    }
	}
    }
