...
if "%FEATURE%" == "HUGE" (
    ...
    nmake -f Make_mvc2.mak CPU=i386 OLE=no GUI=yes IME=yes MBYTE=yes ICONV=yes DEBUG=no FEATURES=%FEATURE% || exit 1
    nmake -f Make_mvc2.mak CPU=i386 OLE=no GUI=no IME=yes MBYTE=yes ICONV=yes DEBUG=no FEATURES=%FEATURE% || exit 1
) 
